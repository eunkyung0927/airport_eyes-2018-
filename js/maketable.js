 ...
 */
var beforecurDate = Date.now() + 7 * 60 * 60 * 1000;
var aftercurDate = Date.now() + 33 * 60 * 60 * 1000;

function maketable(objectType, option) {
  var tb;
  var rawArr = [];//원본데이터의 columns에 해당하는 값들 저장 
  var url = "http://waste.me:1344/V2/TableData?siteId=n3n&query=" + makeTableOption(objectType, option.urlOption);
  $.ajax({
    url: url,
    dataType: "json",
    async: false,
